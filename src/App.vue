    ...
    <div class="app-body">
        ...
        <div class="form-body" v-if="user.username">
          <form v-on:submit.prevent autocomplete="off">
            <div class="input" v-bind:class="{ offline: offline }">
              <input  class="message-input" v-model="form.planElementName"  autocomplete="off" placeholder="Enter your element name..." />
            </div>
            <button @click="sendPlanElement">Send</button>
      ...
      <div v-if="user.username">
        ...
        <div class="chatty-container" v-if="!loading" v-bind:class="{ offline: offline }">
          ...
          <div class="chatty">
            <div v-for="elem of sorted" :key="elem.id">
                <div>{{ elem.name }} - {{ elem.type }})</div>
              </div>
            </div>
          </div>
        </div>
...
import { PlanElement} from "./models";
...
export default {
  ...
  computed: {
    sorted() {
      return [...this.elements].sort((a) => -a.name);
    }
  },
  created() {
    ...
    this.subscription = DataStore.observe(PlanElement).subscribe(elm => {
      console.log(elm.model, elm.opType, elm.element);
  ...
  methods: {
    ...
    loadPlanElements() {
      try {
        this.loading = true;
        DataStore.query(PlanElement, Predicates.ALL).then(elements => {
          this.loading = false;
          this.elements = elements;
        });
      } catch(error){
        console.log("Error loading plan elements...", error);
      } finally {
        this.loading = false;
      }
    },
    sendPlanElement() {
      const { planElementName } = this.form
      if (!planElementName) return;
      DataStore.save(new PlanElement({
    ...
    async deleteAll() {
      await DataStore.delete(PlanElement, Predicates.ALL)
      .then(() => this.loadPlanElements())
      .catch(e => {
        console.log('error deleting all plan elements...', e);
      });
    }, 
    destroyed() {
